...
public class CompleteSearchDecisionRuleMiner extends Miner {
	...
	private final Integer seedBlockToCheck;//null= check all, otherwise = index of only seed block to check
	private final Integer nrOfSeedBlocks;//determines the block size of seeds
	...
	public CompleteSearchDecisionRuleMiner(Log log, KnowledgeBase kb, Rule[] seeds,
		...
		super(log);
		if(seedBlockToCheck != null
				&& (seedBlockToCheck < 0 || seedBlockToCheck >= seeds.length))
			throw new IllegalArgumentException();
		if(nrOfSeedBlocks != null
				&& (nrOfSeedBlocks < 0 || nrOfSeedBlocks > seeds.length))
			throw new IllegalArgumentException();
		if(nrOfSeedBlocks == null)
			nrOfSeedBlocks = seeds.length;
		this.seeds = seeds;
	...
	public List<Rule> execute() throws Exception {
		...
		if(nrOfSeedBlocks != null)
			blocksize = (int) Math.ceil(((double) seeds.length) / nrOfSeedBlocks);
		RulePopulation resultPop = new RulePopulation();
		long t_start_total = System.currentTimeMillis();
		for(int i1 = 0; i1 < seeds.length; i1++) {
			if(seedBlockToCheck == null
					|| (seedBlockToCheck != null
					&& i1 >= seedBlockToCheck*blocksize
					&& i1 < (seedBlockToCheck+1)*blocksize)) {
				Integer currentBlock = seedBlockToCheck;
				if(seedBlockToCheck == null)
						currentBlock = Math.floorDiv(i1, blocksize);
				IntegrationReport report = new IntegrationReport();
				System.out.println(Config.LEVEL_SPACES + "Mining seed " + Math.min(i1+1,seeds.length) + "/" + seeds.length + " starting...");
				File preloadedResultsSeedX = new File(Config.PRELOADING_COMPLETE_RULESSEED + currentBlock + "_"
				...
				if(preloadedResultsSeedX.exists()) {//use preloaded results...
					...
					try {
						...
						System.out.println(Config.LEVEL_SPACES + "Loaded seed block " + currentBlock);
						//set i1 to end of this block
						i1 = ((currentBlock + 1) * blocksize) - 1;
					} catch (Exception ex) {
					...
					for(int i2 = 0; i2 < allDataCombinations.size(); i2++) {
						if((100d*(i2+1))/allDataCombinations.size() >= nextPercentageToShow) {
							System.out.println(Config.LEVEL_SPACES + Config.LEVEL_SPACES
									+ "Mining seed " + Math.min(i1+1,seeds.length) + "/" + seeds.length
				...
				System.out.println(Config.LEVEL_SPACES + "Mining seed " + Math.min(i1+1,seeds.length) + "/" + seeds.length + " finished in "
						+ (t_end-t_start) + "ms");
		...
		if(seedBlockToCheck != null) {//to make parallel computation possible
			System.out.print("Exporting ResPop...");
			ObjectOutputStream out = null;
			try {
				File preloadedResultsSeedX = new File(Config.PRELOADING_COMPLETE_RULESSEED + seedBlockToCheck + "_"
						+ Config.MINIMAL_CONFORMANCE + "_" + Config.MINIMAL_SUPPORT + "_" + Config.MAX_BRANCHING_LEVEL
						+ Config.PRELOADING_EXT);
				...
				throw new RuntimeException("An error occurred when creating the results for the complete search seed block "
						+ seedBlockToCheck + "!", ex);
			}

...
public class DeciClareMinerV11 extends KnowledgeBaseMiner {
	...
	public ArrayList<Rule> execute() throws Exception {
		...
		//		//TODO test
		//		File preloadedResultsPhase1X = new File(Config.PRELOADING_PREFIX + getLog().getName()
		//				+ "_ResP1X_" + Config.MINIMAL_CONFORMANCE + "_" + Config.MINIMAL_SUPPORT + "_" + Config.MAX_BRANCHING_LEVEL
		//				+ Config.PRELOADING_EXT);
		//		ArrayList<Rule> newTest = new ArrayList<>();
		//		newTest.add(rules_phase1.get(6));
		//		ObjectOutputStream out = null;
		//		try {
		//			out = new ObjectOutputStream(new FileOutputStream(preloadedResultsPhase1X));
		...
		if(!Config.PHASE2_DO_COMPLETESEARCH)
			...
			gm = new CompleteSearchDecisionRuleMiner(
					getLog(), getKB(), seedsP2, seedBlockToCheck, nrOfSeedBlocks);
		long t_start = System.currentTimeMillis();

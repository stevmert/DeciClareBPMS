...
public class KnowledgeBase implements Serializable {
	...
	private final HashMap<DataAttribute, HashSet<DataAttribute>> dataElementCooccurences;
	...
	public KnowledgeBase(Log log) {
		...
		this.dataElementCooccurences = new HashMap<>();
		...
		for(Trace t : log) {
			...
			ArrayList<DataAttribute> usedDEs = new ArrayList<>();
			for(DataEvent de : t.getDataEvents()) {//dataElements
				if(dataElementsWithAllUsedValues.add(de.getDataElement()))
					dataElementCooccurences.put(de.getDataElement(), new HashSet<>());
				if(names.add(de.getDataElement().getName()))
					dataElements.add(de.getDataElement());
				usedDEs.add(de.getDataElement());
			}
			for(DataAttribute da : usedDEs) {
				ArrayList<DataAttribute> tmp = new ArrayList<>(usedDEs);
				tmp.remove(da);
				dataElementCooccurences.get(da).addAll(tmp);
			}
			if(Config.MINE_ATLEAST || Config.MINE_ATMOST || Config.MINE_RELATION)
		...
		//save memory by reusing same hashset for DEs with same associated hashset
		HashSet<DataAttribute> todo = new HashSet<>(dataElementCooccurences.keySet());
		for(DataAttribute da1 : dataElementCooccurences.keySet())
			if(todo.contains(da1)) {
				todo.remove(da1);
				HashSet<DataAttribute> set1 = dataElementCooccurences.get(da1);
				for(DataAttribute da2 : new HashSet<>(todo)) {
					HashSet<DataAttribute> set2 = dataElementCooccurences.get(da2);
					if(set1.size() == set2.size()
							&& set1.equals(set2)) {
						todo.remove(da2);
						dataElementCooccurences.put(da2, set1);
					}
				}
			}
	}
	...
	public HashMap<DataAttribute, HashSet<DataAttribute>> getDataElementCooccurences() {
		return dataElementCooccurences;
	}
}
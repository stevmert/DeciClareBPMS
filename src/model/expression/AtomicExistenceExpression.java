...
public class AtomicExistenceExpression implements LogicalExpression, ExistenceExpression, Serializable {
	...
	public boolean hasAtLeast() {
		return this.getExistenceConstraint() instanceof AtLeast;
	}

	@Override
	public HashSet<Activity> getUsedActivities() {
		return exCon.getUsedActivities();
	}

	public static ExistenceExpression parseAtomicExistenceExpression(String input, ParsingCache pc) {
		return new AtomicExistenceExpression((ExistenceConstraint) Constraint.parseConstraint(input, pc));
	}
}
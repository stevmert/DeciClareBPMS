...
public class Decision implements Serializable {
	...
	public HashSet<DataAttribute> getUsedDataAttributes() {
		...
		for(DecisionRule dr : rules)
			res.addAll(dr.getDataValues());
		return res;
	}

	public static Decision parseDecision(String input, ParsingCache pc) {
		Decision d = new Decision();
		for(String x : input.split("\\]"))
			d.addRule(DecisionRule.parseDecisionRule(x.substring(x.indexOf("[")+1), pc));
		return d;
	}
}
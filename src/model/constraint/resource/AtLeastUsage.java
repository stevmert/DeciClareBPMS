...
public class AtLeastUsage extends ResourceUsage {
	...
	public AtLeastUsage(Decision activationDecision, Decision deactivationDec, Activity activity,
	...
	protected String getTextualConstraint() {
		return getActivity() + " uses at least " + getBound() + " " + getResourceExpression();
	}

	@Override
	public Constraint getDecisionlessCopy() {
		return new AtLeastUsage(null, null, getActivity(), getResourceExpression(),
				getBound(), isOptional());
	}
		...
		if(activationTime != -1) {
			...
			for(ActivityEvent ae : actsRem)
				if(ae.equals(getActivity())) {
					...
					if(nrUsed < getBound()) {
						if(ae.getStart() == currentTime)//allow because will be reevaluated when executing the activity

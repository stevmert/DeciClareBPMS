...
public class AtLeastLag extends RelationConstraint implements BoundedConstraint {
		...
		if(activationTime != -1) {//not activated by activation decision
			...
			if(indexCond != null) {
				while(true) {
					...
					if(indexConseq == null)
						break;
					indexConseq.setIndex_end(indexConseq.getIndex_end() + indexCond.getIndex_end() + 1);
					indexConseq.setIndex_start(indexConseq.getIndex_start() + indexCond.getIndex_end() + 1);
					while(true) {
						IndexResult indexCond2 = indexOf(getConditionExpression(), acts, indexCond.getIndex_start()+1, -1);
						if(indexCond2 != null && acts.get(indexCond2.getIndex_start()).getStart() < acts.get(indexConseq.getIndex_start()).getStart())
							indexCond = indexCond2;
						else
							break;
					}
					long lag = acts.get(indexConseq.getIndex_start()).getStart() - acts.get(indexCond.getIndex_end()).getEnd();
					if(lag < atLeast) {
						if(acts.get(indexConseq.getIndex_end()).getEnd() > currentTime) {
							long delay = atLeast - (currentTime - acts.get(indexCond.getIndex_end()).getEnd());
							if(maxDelay == -1 || maxDelay < delay)
								maxDelay = delay;
							countNrOfTimeViolations++;
						} else
							countNrOfViolations++;
					}
	...
	public static AtLeastLag parseConstraint(String input, ParsingCache pc, Decision activationDec, Decision deactivationDecision, boolean isOptional) {
		String text = input.substring(AtLeastLag.class.getSimpleName().length()+1, input.length()-1);
		String[] split = getSplit(text, 3);
		return new AtLeastLag(activationDec, deactivationDecision,

...
public class AtMostLag extends RelationConstraint implements BoundedConstraint {
	...
	public static AtMostLag parseConstraint(String input, ParsingCache pc, Decision activationDec, Decision deactivationDecision, boolean isOptional) {
		String text = input.substring(AtMostLag.class.getSimpleName().length()+1, input.length()-1);
		String[] split = getSplit(text, 3);
		return new AtMostLag(activationDec, deactivationDecision,

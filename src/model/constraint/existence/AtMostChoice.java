...
public class AtMostChoice extends ExistenceConstraint implements TimedConstraint, BoundedConstraint {
	...
	public static AtMostChoice parseConstraint(String input, ParsingCache pc, Decision activationDec, Decision deactivationDecision, boolean isOptional) {
		String text = input.substring(AtMostChoice.class.getSimpleName().length()+1, input.length()-1);
		String[] split = getSplit(text, 4);
		return new AtMostChoice(activationDec, deactivationDecision,
